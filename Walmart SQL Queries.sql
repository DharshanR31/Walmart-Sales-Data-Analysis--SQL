...
SELECT product_line, SUM(tax_5_per) as VAT 
...
SET product_category = 
    (SELECT CASE 
               ...
               ELSE 'Bad' 
           END
     FROM (SELECT AVG(total) AS avg_total FROM sales) AS avg_sales) ;
...
SELECT city, SUM(tax_5_per) AS total_VAT
FROM sales GROUP BY city ORDER BY total_VAT DESC LIMIT 1;

-- 4.Which customer type pays the most in VAT?
SELECT customer_type, SUM(tax_5_per) AS total_VAT
